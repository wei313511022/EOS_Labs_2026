...
/* Fixed Overbug facility layout shared by the Lab 3 server implementation. */

enum component {
	/* Used when a facility does not represent a material component. */
	COMP_NONE = -1,
	COMP_RESISTOR = 0,
	COMP_CAPACITOR = 1,
	COMP_INDUCTOR = 2,
 ...
 *
 * Coordinates are logical pixels in a 1280 by 720 world. (0, 0) is its
 * top-left corner; x increases to the right and y increases downward. x and y
 * identify the top-left corner of a facility rectangle, while w and h are its
 * width and height. Only material bins use component; all other types use
 * COMP_NONE.
 *
 * Keep this list aligned with the Lab 3 facility contract. server.c expands it
 * to build its runtime station array, so this header is the authoritative map.
 */
